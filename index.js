...
// If explicit coin trading, autoselling new pairs will be disabled by default.
...
binanceRequest.onload = function () {
    const local_symbol = {};
    let binanceResponse = JSON.parse(binanceRequest.responseText);
    for (let k = 0; k < binanceResponse.length; k++) {
        //Update Symbol Pairs
...
function getCoinMarketCapDetails() {
    if (global_cmcapikey.match(/\S/)) {
        ...
        cmcRequest.onload = function () {
            ...
            for (let y = 0; y < ourDatac.data.length; y++) {
...
function checkIfTradable() {
    ...
    exchangeInfoRequest.onload = function () {
        let exchangeInfoResponse = JSON.parse(exchangeInfoRequest.responseText);
        for (let x = 1; x < exchangeInfoResponse["symbols"].length; x++) {
...
function TradeHelper() {
    ...
    let local_coin_existing_bal;

    if ($('#fiatTo option:selected').val() === fiat_currency) {
        ...
        binanceRest1.account()
            .then((accountInfo1) => {
                for (let l = 0; l < accountInfo1["balances"].length; l++) {
                    if (accountInfo1["balances"][l]["asset"] === local_new_coin) {
                        local_coin_existing_bal = parseFloat(accountInfo1["balances"][l]["free"]);
                        break;
                    }
                }
                ...
                global_initial_price = sell_coin["coinprice"]; // To calculate overall change
                global_peak = sell_coin["coinprice"]; // To track the prices. Helpful in trial stop.
                ...
                */
                sell_coin["quantity"] = local_coin_existing_bal;

                global_markup = sell_coin["quantity"] + "(" + sell_coin["coin"] + ") -> ";
...
function exitTradeNowFunc() {
    ...
    let local_fiat_existing_bal, local_fiat_bal_after_order;
    ...
    binanceRest1.account()
        ...
        .then((accountInfo2) => {
            ...
            for (let n = 0; n < accountInfo2["balances"].length; n++) {
